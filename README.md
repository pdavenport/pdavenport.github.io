...
This project now requires Node.js 20.9+ overall because the upgraded `next` and `sharp` dependencies no longer support older Node releases. On 32-bit Windows, Sharp’s optional image-optimization package is still the limiting dependency for future Node major-version upgrades because it currently advertises `^20.9.0` support on that platform.
