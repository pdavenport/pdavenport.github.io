...
This project now requires Node.js 20.9+ overall because the upgraded `next` and `sharp` dependencies no longer support older Node releases. On 32-bit Windows only, Sharp’s optional image-optimization package currently advertises `^20.9.0` support, so that specific platform should stay on Node 20.x for now.
...
- `minimatch` 3, `minimatch` 9, and `@typescript-eslint/typescript-estree -> brace-expansion` for the range-expansion DoS advisories; remove once those parents ship patched `brace-expansion` paths
